...
make()
```
ch1:=make(chan int) //无缓冲区 同步channel
ch2:=make(chan string,1) //缓冲区 异步channel
```
...
- `x,ok:=<-ch` ok is false当缓冲区中没有更多的值可以接收(缓冲区为空)并且通道被关闭。

第二个变量经常理解为管道的关闭状态，这是不准确的。管道的关闭有两种状态，缓冲区已经没有数据，缓冲区还有数据。
